...
test('sports routes and selection controls work', async ({ page }, testInfo) => {
  ...
  if (!testInfo.project.name.includes('mobile')) {
    ...
    await expect(page.getByText('演示盘口已生成，不会提交真实订单。')).toBeVisible();
  }
});
...
test('combo builder calculates and previews selected legs', async ({ page }) => {
  ...
  await expect(page.getByText('演示组合已生成，不会提交真实订单。')).toBeVisible();
});
...
test('perps asset tabs, range and preview work', async ({ page }) => {
  ...
  await expect(page.getByText('演示永续意图已生成，不会提交资金或订单。')).toBeVisible();
});
